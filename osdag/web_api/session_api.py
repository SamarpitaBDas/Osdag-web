...
class CreateSession(APIView):
    ...
    def create_new_session(self, module_id):
        pass
    ...
    def post(self,request) :
        ...
        if request.COOKIES.get("connection_session") is not None: # Error Checking: Already editing design.
            print('Cookie is there')
            
            cookie_id = request.COOKIES.get("connection_session")
            try:
                design = Design.objects.get(cookie_id=cookie_id)
                
                if design.module_id != module_id:
                    # TODO: Handle Multiple
                    return JsonResponse({
                        "status" : "set"
                    }, status=211) # Returns error response.
                    
                return JsonResponse({
                    "status" : "set", "module_name": design.module_id.replace(" ", "-")}, status=200) # Returns error response.
        ...
        if serializer.is_valid() : 
            ...
            response.set_cookie(key = "connection_session", value = cookie_id , samesite = 'None' , secure = 'True') # Set session id cookie.
            return response
        else : 
...
class DeleteSession(APIView):
    ...
    def post(self,request: HttpRequest) -> HttpResponse:
        cookie_id = request.COOKIES.get("connection_session") # Get design session id.
        ...
        response.delete_cookie("connection_session")
        return response